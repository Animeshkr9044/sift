...
JSON_FILE = 'data/zomato_reviews.json'
DB_FILE = 'data/zomato_reviews.db'
TABLE_NAME = 'zomato_reviews'

def create_table_and_load_data():
    """Reads Zomato reviews from a JSON file and loads them into a new SQLite database."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()

    # Create the new table for Zomato reviews
    cursor.execute(f'''
